...
3. **输入元件: Button(按钮), Dip Switch(拨码开关)**
   **Button**： 当按下鼠标按钮时，组件的输出将为 1。释放鼠标按钮时，输出将恢复为 0。不过我还发现可以在**状态栏中的属性中来改变它的按下逻辑**。
![](./Button.png)
   **Dip Switch**:Dip Switch 作为一个输入组件接入电路。它用于提供一个可操作的开关状态，模拟用户的输入。每个开关的状态（开或关）相当于一个 二进制输入信号。它可以外接其他电路元件,**只有在点击“更改回路内的值”这个按键之后，在点击 Dip Switch中的小按钮，才会变化输入输出。**
![](./Dip_switch.png)

附：我的logisim中Dip Switch不能查看参考资料，是使用AI来解释说明怎么使用的。
4. **输出元件: LED, 7-Segment Display(7段数码管)**
   **LED**:根据输入是 1 还是 0，通过对 LED 进行着色（由其 Color 属性指定）来显示其输入的值。
   使用方法如下：
![](./LED.png)
    **7-Segment Display**:显示其八个一位输入的值，跟LED的呈现形式作用很相似。
